...
int main()
{
    ...
    unsigned char* data = stbi_load(std::string("../images/container.jpg").c_str(), &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    // alpha settings
    glUniform2f(glGetUniformLocation(ourShader.ID, "alpha"), 0.5, 0.0);
    float alpha = 0.5;
    // render loop
    // -----------
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS)
            alpha -= 0.01;
        glUniform2f(glGetUniformLocation(ourShader.ID, "alpha"), alpha, 0.0);
        // render
        // ------
        ...
        // render container
        trans = glm::mat4(1.0f);
        trans = glm::rotate(trans, float(0.1 * glfwGetTime()), glm::vec3(0.0f, 0.0f, 1.0f));
